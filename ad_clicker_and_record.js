...
assetNumber = Math.floor(Math.random() * totalAgents);

let assetBase = `https://syntheticmessenger.s3.us-east-1.amazonaws.com/${assetNumber}/`;
...
async function clickAds(page, url) {
  ...
  const outname = `${new Date().getTime()}.mp4`;
  // const cmd = `ffmpeg -f avfoundation -r 60 -i "5:2" -filter:v "crop=${width*2}:${height*2}:0:46" -pix_fmt yuv420p -n ${outname}`;
  ...
  const options = {
    ...
    showCursor: false,
    audioDeviceId: "com.vbaudio.vbcable:B6E6D02E-CD32-4C95-B60B-2FC2621CA4E0",
  };
...
async function main(urls) {
  ...
  for (let url of urls) {
    ...
    assetBase = `https://syntheticmessenger.s3.us-east-1.amazonaws.com/${assetNumber}/`;
  }

  if (runForever && runs < 4) {
    main([]);
    runs ++;
  }
}
